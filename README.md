...
Тестирование проекта не автоматизированно, использовал утилиту Postman (https://www.getpostman.com/), из нее сделал выгрузки запросов сформированные на php cURL, посмотреть можно в файле
```
basic/test/TestRequests.php
```
...
Что хотел бы еще добавить
------------------
- квоты на дисковое пространство пользователя (я вижу это как доп. поле для пользователя в базе с остатком его места на диске, при добавлении нового файла уменьшаем квоту на размер файла);
- ограничении доступа к файлам можно попробовать реализовать через flock(), это важно, т.к. можно авторизоваться под одним логином и попытаться получить доступ к одному файлу - это нежелательно);
- ограничение на количество запросов к API для клиента и/или ip адреса (в Yii можно реализовать через RateLimitInterface());
- работу с большими файлами теоретически можно попробовать через реализованный метод потоковой загрузки (PUT) файла (не тестировал);
- https - шифрование передаваемых файлов от атак на приложение;
- кеширование важно для нагруженных систем, и быстрого ответа на запросы польззователей;
